...
const defaultAmount = (id: number) => 100 + (id % 3) * 25;
...
export default function MicroGrantsPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const mapped: UiGrant[] = data.map((g) => ({
        ...g,
        amountNeeded: defaultAmount(g.id),
        amountRaised: Math.round(defaultAmount(g.id) * 0.4),
        timePosted: g.created_at
          ? new Date(g.created_at).toLocaleDateString()
          : "today",
        supporters: (g.id % 5) + 1,
        verified: g.id % 2 === 0,
        urgent: g.id % 3 === 0,
        requesterName: `User #${g.requestor_id}`,
      }));
      setGrants(mapped);
    } catch (_) {
      setGrants([]);
  ...
  return (
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-600">
              Support community members with small medical grants under $200
            </p>
          </div>
                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div className="lg:col-span-2 space-y-4">
                          <div>
                            ...
                            <p className="text-gray-600">
                              Requested by {grant.requesterName}
                            </p>
                          </div>
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="text-xs text-gray-500 text-center">
                            All donations are secure and go directly to medical
                            expenses
                          </div>
                        </div>
                      </div>
          ...
          <TabsContent value="my-grants" className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Track your submitted grant requests and their progress
                </CardDescription>
              </CardHeader>
              <CardContent>
                {myGrants.length > 0 ? (
                  ...
                  <div className="text-center py-12">
                    ...
                    <p className="text-gray-600 mb-4">
                      Need help with medical expenses? Request a micro-grant
                      from the community
                    </p>
                    <Button onClick={() => setShowGrantForm(true)}>
                      <Plus className="h-4 w-4 mr-2" />
        ...
        {showGrantForm && (
          <GrantRequestForm onClose={() => setShowGrantForm(false)} />
        )}
      </div>
    </DashboardLayout>
  );
