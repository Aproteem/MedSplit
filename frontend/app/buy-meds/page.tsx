...
export default function BuyMedsPage() {
	...
	const handleSearch = async (override?: string) => {
		...
		try {
			const url = q ? api(`/api/medicines?query=${encodeURIComponent(q)}`) : api(`/api/medicines`)
		...
		try {
			if (!existing) {
				const res = await fetch(api(`/api/wishlists`), {
					...
					body: JSON.stringify({ user_id: user.id, medicine_id: medId, quantity: 1 }),
				})
	...
	const getDemandProgress = (current: number, required: number) => {
		...
		return Math.min(100, Math.max(0, (Number(current) / Number(required)) * 100))
	}
			...
			<div className="space-y-6">
				...
				{searchActive && (
					<div className="space-y-4">
						...
						{results.map((med) => {
							const demandProgress = getDemandProgress(med.current_demand, med.required_demand)
											...
											<div className="lg:col-span-2 space-y-4">
												...
												<div className="space-y-2">
													...
													<p className="text-xs text-gray-500">
														{Math.max(0, Number(med.required_demand) - Number(med.current_demand))} more
														people needed for bulk order
													</p>
												</div>
											</div>
